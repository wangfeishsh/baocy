...
基本的想法是尽可能在一个事务中延迟提交所有资源，，使唯一可能出错的是基础设施故障（而不是一个业务处理错误）。系统依靠Best Efforts 1PC原因，基础设施故障是罕见的，他们可以在高吞吐量的回报承担风险。如果业务处理服务的设计也是幂等的，那么可以在实践中出些小错。
...
为了帮助您更好地理解模式并分析失败的后果，我将使用消息驱动的数据库更新作为一个例子。
...
要点是最后两个步骤是重要的：他们必须在最后一个，在这个顺序。顺序之所以重要的原因是技术上的，但顺序本身是由业务需求决定的。该命令告诉你，在这种情况下的事务资源是特殊的，它包含了关于如何进行其他工作的说明。这是一个业务排序：系统不能自动告诉它如何去做（虽然消息和数据库是两种资源，那么它往往是在这个顺序）。在失败的情况下排序是重要。最常见的失败的情况（到目前为止）是一个失败的业务处理（坏数据，编程错误，等等）。在这种情况下，两个事务都可以很容易地被操纵以响应一个异常和回滚。在这种情况下，业务数据的完整性被保留，与时间轴类似于本文在本文开头所概述的理想故障案例。
...
触发回滚的精确机制是不重要的，有几个是可用的。重要的一点是，提交或回滚发生在资源的业务排序的顺序相反。在示例应用程序中，消息传递事务必须提交，因为业务流程的指令包含在该资源中。这是很重要的，因为（罕见）失败的情况下，第一次提交成功，第二个失败。由于设计所有的业务处理已经完成，在这一点上，这样的部分故障的唯一原因将是一个基础设施问题与消息中间件。
