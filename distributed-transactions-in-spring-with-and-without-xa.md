...
要点是最后两个步骤是重要的：他们必须在最后一个，在这个顺序。顺序之所以重要的原因是技术上的，但顺序本身是由业务需求决定的。该命令告诉你，在这种情况下的交易资源是特殊的，它包含了关于如何进行其他工作的说明。这是一个商业排序：系统不能自动告诉它的方式轮（虽然消息和数据库是两种资源，那么它往往是在这个顺序）。排序是重要的原因与失败的情况下。最常见的失败的情况（到目前为止）是一个失败的业务处理（坏数据，编程错误，等等）。在这种情况下，两个事务都可以很容易地被操纵以响应一个异常和回滚。在这种情况下，业务数据的完整性被保留，与时间轴类似于本文在本文开头所概述的理想故障案例。
