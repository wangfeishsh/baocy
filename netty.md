...
一个[ChannelHandlerContext](http://netty.io/4.0/api/io/netty/channel/ChannelHandlerContext.html)对象提供了各种操作可以触发各种I\/O事件和操作。在这里，我们调用write\(Object\)在逐字写收到的消息。请注意，我们没有释放收到的消息，不像我们在DISCARD的例子。这是因为当它被写出到wire时，Netty为你释放它。

CTX。写（对象）不把消息写出来的线。这是内部缓冲，然后冲到电线flush\(\) CTX。或者，你可以打电话给CTX。writeandflush（MSG）简洁。
