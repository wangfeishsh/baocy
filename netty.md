...
3. 要实现DISCARD协议，handler必须忽略接收的消息。[ByteBuf](http://netty.io/4.0/api/io/netty/buffer/ByteBuf.html)是引用计数的对象已被释放，通过显式release\(\)方法。请记住，它是handler的责任，释放任何传递给handler的引用计数的对象。通常，channelread\(\)处理实现方法如下
