...
从控制台写入数据，并将其写入控制台是一个方便的开始，但你可能会希望使用其他来源的数据或从Kafka导出数据到其他系统。对于许多系统，不需要编写自定义的集成代码，您可以使用Kafka Connect导入或导出数据。Kafka Connect是一个工具，包括与Kafka的导入或导出数据到Kafka。它是一个可扩展的工具，运行连接器，它实现与外部系统交互的自定义逻辑。在这个示例中我们将看到怎样运行Kafka Connect简单的连接器，从一个文件导入数据到Kafka主题和从Kafka主题导出数据到文件。首先，我们将开始通过创建一些种子数据来测试
...
这些示例配置文件，包括Kafka，默认使用前面的本地集群配置并创建两个连接器：第一是资源连接器，读取输入文件的行并将每行记录到Kafka的主题，二是水槽连接器，从Kafka的主题读取信息和产出每一行到输出文件中。在启动过程中你会看到许多的日志信息，包括连接器实例化的一些指示说明。一旦Kafka Connect进程已经启动，源连接器应该从
