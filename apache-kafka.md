...
许多人使用Kafka作为一个日志集成解决方案的替代。日志聚合通常收集物理日志文件的服务器放在一个中心位置（文件服务器或HDFS也许）处理。卡夫卡抽象的文件的细节，并给出了一个干净的抽象的日志或事件数据作为一个流的消息。这允许较低的延迟处理和更容易支持多个数据源和分布式数据消耗。在比较，以日志为中心的系统，如书记或水槽，卡夫卡提供同样良好的性能，更强的耐用性保证，由于复制，和低得多的终端到终端的延迟。
