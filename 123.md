...
再按照Spring的哲学，TransactionException可以由任何未经检验的PlatformTransactionManager接口方法抛出（即，它继承了java.lang.RuntimeException类）。交易基础设施的故障几乎总是致命的。在极少数情况下，应用程序代码可以从事务故障恢复，应用程序开发人员仍然可以选择捕获和处理transactionexception。突出的一点是，开发商没有被迫这样做。
